...
def dispensa(d):
    ...
    d.p("Su una rete locale, quando un computer vuole parlare con un altro, chiede via ARP "
        "'chi ha questo IP?' e si fida della prima risposta. Non c'e' nessun controllo. "
        "Un attaccante puo' rispondere 'ce l'ho io!' anche se non e' vero, e da quel "
        "momento il traffico destinato a quell'IP arriva a lui. Se si mette tra due "
        "interlocutori, li ascolta entrambi: e' il man-in-the-middle, alla base di molti "
        "attacchi su wifi e reti aziendali.")

    d.h2("ARP non verifica niente")
    d.p("L'ARP e' nato per essere semplice e veloce, in tempi in cui la rete era fidata. "
        "Non ha autenticazione: chiunque puo' mandare una risposta ARP falsa (spoofing) e "
        "avvelenare la cache degli altri. Questo difetto e' ancora sfruttabile su moltissime "
        "reti locali.")
    ...
    d.code([
        ...
        "# tra i pacchetti del bersaglio verso 'il server' c'e' la flag",
    ])
    ...
    d.box("blu", "Per un MITM completo (teoria)", items=[
        ...
        "Cosi' l'attaccante e' invisibile: le due parti continuano a parlarsi tramite lui.",
        "Se il traffico e' cifrato (HTTPS), l'attaccante vede passare i dati ma non li "
    ...
    d.p("Etica e legge: l'ARP spoofing intercetta le comunicazioni altrui. Farlo fuori dal "
        "laboratorio e' un reato. Qui si fa solo tra le nostre due VM isolate.")
...
def manuale(d):
    ...
    d.bullets([
        "Capire perche' l'ARP e' spoofabile e cosa comporta il MITM.",
    ...
    d.bullets([
        ...
        "L'arpspoof dice alla vittima che 10.10.10.30 ha il MAC della Kali: da li' i "
    ...
    d.table(["Sintomo", "Causa e rimedio"], [
        ...
        ["il traffico si ferma", "l'ARP va rinfrescato: l'arpspoof rimanda ogni 2s (gia' fatto)"],
    ], widths=[3000, 6026])
