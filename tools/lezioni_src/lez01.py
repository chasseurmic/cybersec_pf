...
def dispensa(d):
    d.box("blu", "In breve", [
        ...
        "perche' in questo corso studiamo il punto di vista dell'attaccante, e muovere i "
        "primi passi nel terminale della Kali.",
        "**Al termine sai:** cos'e' la triade CIA, le regole etiche e legali del corso, e "
        "i comandi base per orientarti nel terminale (chi sei, dove sei, cosa c'e').",
    ...
    d.p("Ogni giorno aziende, ospedali e scuole vengono attaccati: dati rubati, siti "
        "bloccati, richieste di riscatto. Dietro quasi ogni notizia c'e' la stessa storia: "
        "qualcosa che doveva restare segreto e' stato letto, qualcosa che doveva restare "
        "intatto e' stato modificato, oppure un servizio che doveva restare acceso e' "
        "stato spento. Questi tre 'doveva' sono esattamente cio' che la sicurezza "
        "informatica cerca di proteggere.")

    d.h2("La triade CIA")
    d.p("CIA non c'entra con i servizi segreti: e' l'acronimo dei tre pilastri della "
        "sicurezza. Ogni attacco colpisce almeno uno di questi tre.")
    d.table(["Pilastro", "Cosa garantisce", "Esempio di violazione"], [
        ["Confidenzialita' (C)", "solo chi ha diritto puo' leggere i dati",
         "un attaccante ruba l'elenco delle password"],
        ["Integrita' (I)", "i dati non vengono alterati di nascosto",
         "qualcuno modifica un voto o un bonifico"],
        ["Disponibilita' (A)", "il servizio e' accessibile quando serve",
    ...
    d.p("Tenere a mente la triade aiuta a ragionare: davanti a qualunque scenario, "
        "chiediti quale pilastro e' a rischio. E' la bussola di tutto il corso.")

    d.h2("Perche' studiamo il punto di vista dell'attaccante")
    d.p("Per difendere una casa devi sapere come entra un ladro: quali finestre lascia "
        "aperte la gente, quali serrature sono deboli. In sicurezza e' lo stesso. Impariamo "
        ...
        "difesa). Alla fine di ogni lezione c'e' sempre un 'ribaltamento difensivo': la "
    ...
    d.box("rosso", "Regola d'oro (etica e legge)", items=[
        ...
        "Attaccare, scansionare o provare password su computer altrui e' un REATO "
        ...
        "portano in tribunale. La differenza e' il permesso.",
    ])
    ...
    d.p("Lavorerai con due macchine virtuali isolate: la **Kali** (il computer "
        ...
        "dentro il tuo PC: non tocca internet ne' la rete della scuola. La Kali ha "
        "indirizzo `10.10.10.5`, il bersaglio `10.10.10.20`.")

    d.h1("Parte 2 · Primi passi nel terminale (pratica, 80 min)")
    d.p("Il terminale e' la finestra dove si scrivono i comandi. Fa un po' paura all'inizio, "
        "ma e' solo un modo per dire al computer cosa fare a parole invece che a clic. Sulla "
    ...
    d.p("I primi comandi servono a orientarsi: chi sei, che indirizzo hai, cosa c'e' "
        "intorno a te.")
    d.cmdref([
        ...
        ("`ip a`", "Elenca le schede di rete e i loro indirizzi IP (`a` sta per address). "
                   "Cerca l'indirizzo `10.10.10.5`: e' la tua Kali nella rete del lab."),
    ...
    d.h2("Guardare cosa c'e' e leggere i file")
    d.cmdref([
        ...
        ("`cat`", "Concatena e stampa a schermo il contenuto di uno o piu' file di testo."),
    ])

    d.h3("Flag 1 · Il file di benvenuto (+20)")
    d.p("Nella tua cartella c'e' un file di benvenuto. Elenca i file e leggilo.")
    d.code([
        "ls",
        "cat README-corso.txt",
    ])
    d.p("In fondo al file c'e' la prima flag (una scritta tipo `FLAG{...}`). Le flag sono "
        "il modo con cui dimostri di aver completato un compito: le consegni al docente per "
        "guadagnare punti. Non rivelano nulla di segreto qui, ma piu' avanti dovrai proprio "
        "guadagnartele.")
    ...
    d.p("I file il cui nome inizia con un punto (per esempio `.segreto`) non compaiono con "
        ...
        "`-a` li vedi tutti. Ricorda questa lezione: **nascondere non e' proteggere**.")
    d.code([
        "ls -a",
        "cat .segreto",
    ])
    ...
    d.cmdref([
        ("`ping`", "Verifica se un altro computer e' raggiungibile in rete e quanto ci "
    ...
    d.box("verde", "La triade all'opera", items=[
        "La flag nascosta (`.segreto`) insegna che nascondere non e' proteggere: la "
        "confidenzialita' vera si ottiene coi permessi e la cifratura, non nascondendo.",
        "La flag nella pagina web ricorda che tutto cio' che il server manda al client puo' "
...
def manuale(d):
    ...
    d.table(["Sintomo", "Causa e rimedio"], [
        ["curl non risponde", "il bersaglio e' spento o `lab 1` non e' stato lanciato sul "
         "bersaglio; verificare la pagina con `docker ps` (container `banca`)"],
        ["ip a non mostra 10.10.10.5", "la scheda 'Rete interna labnet' della Kali non e' "
         "attiva; vedi Lezione 2"],
        ["rigiocare", "rilanciare `lab 1` su Kali e bersaglio"],
    ], widths=[2800, 6226])
    d.h1("Nota didattica")
    d.p("E' la prima volta al terminale per molti: andare piano, far digitare tutti, "
