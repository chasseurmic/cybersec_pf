...
def dispensa(d):
    ...
    d.p("Quando mandi un messaggio su WhatsApp, paghi online o apri un sito con il "
        "lucchetto, dietro c'e' la crittografia. Serve a due cose: tenere segreto un "
    ...
    d.h2("Cifrare non e' fare l'hash")
    d.table(["", "Hash", "Cifratura"], [
        ["Reversibile?", "no, a senso unico", "si, con la chiave giusta"],
        ["A cosa serve", "verificare integrita', conservare password", "tenere segreto un contenuto"],
    ...
    d.p("Nella cifratura simmetrica (per esempio AES) la stessa chiave cifra e decifra. "
        "Veloce e robusta, ma c'e' un problema: come consegni la chiave all'altra persona "
    ...
    d.p("Nella cifratura asimmetrica (per esempio RSA) ogni persona ha due chiavi legate "
        ...
        "che cifri con la pubblica si apre solo con la privata. Cosi' chiunque puo' "
        ...
        "pubblica che sei stato davvero tu: e' la firma digitale.")
    ...
    d.p("Ti e' stata data la passphrase: `chiavesegreta`. Decifra il file AES.")
    d.code([
        "cd ~/lab/lezione-21",
    ...
    d.p("Il messaggio e' stato cifrato con la chiave pubblica: solo la privata "
        "(`priv.pem`) lo apre.")
    d.code([
    ...
    d.box("verde", "Usare la crittografia bene", items=[
        ...
        "simmetrica, poi simmetrica (veloce) per i dati. E' quello che fa HTTPS, prossima "
        "lezione.",
    ])
...
def manuale(d):
    ...
    d.p("Passphrase simmetrica: `chiavesegreta`. Le flag sono didattiche e in chiaro nel "
        "codice: qui la competenza e' l'operazione di cifratura, non la segretezza del "
        "valore.")
    d.h1("Troubleshooting")
    d.table(["Sintomo", "Causa e rimedio"], [
        ["'bad decrypt'", "passphrase sbagliata: e' `chiavesegreta`; serve `-pbkdf2`"],
